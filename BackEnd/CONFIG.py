...
POSTGRES_INSERT_BATCH_SIZE = 1_000
...
TRACKING_DETECTOR_CONFIDENCE_THRESHOLD = 0.10
TRACKING_MAX_LOST_SECONDS = 3.0
...
class TrackingConfig:
    ...
    # YOLO must emit low-confidence boxes so ByteTrack can associate them with
    # an existing track.  New tracks remain governed by new_track_thresh in
    # the generated ByteTrack YAML (the legacy confidence_threshold default).
    detector_confidence_threshold: float = TRACKING_DETECTOR_CONFIDENCE_THRESHOLD
    confidence_threshold: float = 0.25
    iou_threshold: float = 0.70
    max_detections: int = 300
    batch_size: int = TRACKING_BATCH_SIZE
    max_lost_seconds: float = TRACKING_MAX_LOST_SECONDS
    image_size: int = TRACKING_IMAGE_SIZE
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.detector_confidence_threshold <= 1.0:
            raise ValueError("detector_confidence_threshold must be within [0, 1].")
        if not 0.0 <= self.confidence_threshold <= 1.0:
            raise ValueError("confidence_threshold must be within [0, 1].")
        if self.detector_confidence_threshold > self.confidence_threshold:
            raise ValueError(
                "detector_confidence_threshold must not exceed confidence_threshold."
            )

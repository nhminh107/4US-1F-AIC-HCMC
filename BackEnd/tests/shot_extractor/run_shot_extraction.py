"""Run ShotExtractor on one local MP4 file and save the detected shots as JSON."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[3]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from BackEnd.app.contracts.pipeline import ShotMetadata
from BackEnd.app.shot_extractor.shot_extractor import DEFAULT_WEIGHTS_PATH, ShotExtractor

DEFAULT_RESULT_DIR = PROJECT_ROOT / "BackEnd" / "tests" / "result"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Detect shot boundaries for one MP4 video and save them as JSON."
    )
    parser.add_argument("video_path", type=Path, help="Path to the input .mp4 video.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_RESULT_DIR,
        help=f"Directory for the JSON output (default: {DEFAULT_RESULT_DIR}).",
    )
    parser.add_argument(
        "--weights-path",
        type=Path,
        default=DEFAULT_WEIGHTS_PATH,
        help="Path to the converted TransNetV2 .pth checkpoint.",
    )
    parser.add_argument(
        "--device",
        choices=("cpu", "cuda"),
        default=None,
        help="Inference device. Omit to use CUDA automatically when available.",
    )
    return parser.parse_args()


def _validate_video_path(video_path: Path) -> Path:
    resolved_path = video_path.expanduser().resolve()
    if not resolved_path.is_file():
        raise FileNotFoundError(f"Input video does not exist: {resolved_path}")
    if resolved_path.suffix.lower() != ".mp4":
        raise ValueError(f"Only .mp4 input is supported by ShotExtractor: {resolved_path}")
    return resolved_path


def _shot_to_dict(shot: ShotMetadata) -> dict[str, int | str | None]:
    return {
        "shot_id": shot.shot_id,
        "video_id": shot.video_id,
        "shot_index": shot.shot_index,
        "start_ms": shot.start_ms,
        "end_ms": shot.end_ms,
        "start_frame_idx": shot.start_frame_idx,
        "end_frame_idx": shot.end_frame_idx,
    }


def main() -> None:
    args = _parse_args()
    video_path = _validate_video_path(args.video_path)
    output_dir = args.output_dir.expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    extractor = ShotExtractor(
        weights_path=args.weights_path.expanduser().resolve(),
        device=args.device,
        video_dir=video_path.parent,
    )
    shots = extractor.extract(video_path.stem)

    output_path = output_dir / f"{video_path.stem}_shots.json"
    output_path.write_text(
        json.dumps([_shot_to_dict(shot) for shot in shots], ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )

    print(f"Detected {len(shots)} shots.")
    print(f"Saved results to: {output_path}")
    for shot in shots:
        print(
            f"{shot.shot_id}: {shot.start_ms}-{shot.end_ms} ms "
            f"(frames {shot.start_frame_idx}-{shot.end_frame_idx})"
        )


if __name__ == "__main__":
    main()
